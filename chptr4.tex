...
また，一般に使用されているQRコードでは，同一サイズでも撮影距離80　cmから検出できることから，本実験で使用したコード検出システムは既存のものに大きく劣ることが分かった．
...
ここで，一連の低解像度化処理について解説する．始めに，QRコードにする文字列を3つに分割する．次に，分割された文字列に対し，それぞれRGB単色QRコードに変換する．これにより得られた3つのQRコードを重ね合わせることでカラーQRコードを生成する．デコード時には，RGBそれぞれで得られた文字列を結合することで元の文字列を復元することが可能となる．処理前の文字列をＱＲコードに変換したときには，誤り訂正レベルM(Middle)で,バージョン4(セル数$33 \times 33$)でエンコードされた(図\ref{fig:qr_123_spec})．一方で，3つに分割された文字列を同一の誤り訂正レベルM(Middle)でエンコードした結果，バージョン2(セル数$25 \times 25$)でのエンコードが可能となった(図\ref{fig:qr_1_spec}, \ref{fig:qr_2_spec}, 及び\ref{fig:qr_3_spec})．この時，QRコードへのエンコードにはQRコード生成サイト\footnotetext{\url{https://www.cman.jp/QRcode/qr_make/}}を使用した．
